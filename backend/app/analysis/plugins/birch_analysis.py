"""Birch clustering analysis plugin.

Performs Birch clustering and reports tree parameters.
"""
...
class BirchAnalysis(AnalysisPlugin):
    """Plugin that performs Birch clustering."""

    name = "Birch Clustering"
    description = "Performs Birch clustering and reports branching factor and threshold." 
    ...
    def execute(self, dataset: pd.DataFrame) -> Dict[str, Any]:
        ...
        unique_labels.sort()
        cluster_sizes = {int(int_label): int(np.sum(labels == int_label)) for int_label in unique_labels}
        ...
        results = {
            ...
            "branching_factor": int(getattr(model, "branching_factor", 50)),
    ...
    def explain(self, results: Dict[str, Any]) -> Dict[str, str]:
        return {
            ...
            "clusters": "The number of clusters identified by Birch.",
            "cluster_sizes": "The number of observations assigned to each cluster.",
            "branching_factor": "The branching factor parameter for the Birch tree.",
            "threshold": "The threshold used for merging subclusters in Birch.",
        }
