        ...
        for col, metrics in results.items():
            ...
            class DescriptiveStatistics(AnalysisPlugin):
                ...
                def execute(self, dataset: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
                    ...
                    The method returns only raw statistical results structured as
                    a mapping from column name to metrics. It does not build any
                    response envelope or perform timing/confidence calculations.
                ...
                def explain(self, results: Dict[str, Dict[str, Any]]) -> Dict[str, str]:
                    ...
                    return {
                        ...
                        "interquartile_range": (
                            ...
                            "representing the spread of the middle 50% of observations."
                        ),
                        "count": "Number of non-missing observations used for calculations.",
                        "missing_values": "Number of missing observations in the column.",
                    }
                ...
                def observations(self, results: Dict[str, Dict[str, Any]]) -> Dict[str, List[str]]:
                    ...
                    Returns a mapping where each key is a column name and the value
                    is a list of short, objective observations derived from the stats.
                    """
                    ...
                    for col, metrics in results.items():
                        ...
                        if missing == 0:
                            col_obs.append("No missing values detected.")
                        else:
                            total = count + missing
                            if total > 0 and (missing / total) > 0.2:
                                col_obs.append("Large number of missing values detected.")
                            else:
                                col_obs.append("Some missing values detected.")
                        ...
                        if mean not in (None, 0) and std_dev is not None:
                            cov = abs(std_dev / (abs(mean) + 1e-12))
                            if cov < 0.1:
                                col_obs.append("Values are tightly clustered around the average.")
                            elif cov > 1:
                                col_obs.append("Values show high variability.")

                        # Symmetry observation
                        if mean is not None and median is not None:
                            if abs(mean - median) / (abs(mean) + 1e-12) < 0.05:
                                col_obs.append("Distribution appears symmetric.")
                            else:
                                col_obs.append("Mean differs substantially from median.")
