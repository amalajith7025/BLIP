"""Naive Bayes analysis plugin.

Builds a Gaussian Naive Bayes classifier for binary classification.
"""
...
class NaiveBayesAnalysis(AnalysisPlugin):
    """Plugin that performs Gaussian Naive Bayes classification."""
    ...
    def validate(self, profile: DatasetProfile) -> bool:
        """Return True when the dataset structure is valid for Naive Bayes classification."""
    ...
    def execute(self, dataset: pd.DataFrame) -> Dict[str, Any]:
        """Execute Gaussian Naive Bayes classification on the dataset."""
        ...
        class_priors = {
            ...
            for class_name, prior in zip(encoder.classes_, model.class_prior_)
        }
    ...
    def explain(self, results: Dict[str, Any]) -> Dict[str, str]:
        """Return standardized explanations for Naive Bayes output keys."""
        return {
            "samples_used": "The number of complete observations used to train the Naive Bayes model.",
            "predictors": "The numeric predictor variables included in the Naive Bayes model.",
            ...
            "class_priors": "The prior probability estimated for each target class.",
        }

    def observations(self, results: Dict[str, Any]) -> Dict[str, List[str]]:
        """Generate objective observations from Naive Bayes results."""
        if not results:
            return {"": ["Very small dataset used."]}

        observations: List[str] = []
        accuracy = results.get("accuracy", 0.0)
        class_priors = results.get("class_priors", {})
        ...
        return {"naive_bayes": observations}
