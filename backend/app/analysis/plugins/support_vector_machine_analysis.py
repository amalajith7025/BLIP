"""Support Vector Machine analysis plugin.
...
class SupportVectorMachineAnalysis(AnalysisPlugin):
    """Plugin that performs support vector machine classification."""
    ...
    def validate(self, profile: DatasetProfile) -> bool:
        """Return True when the dataset structure is valid for SVM classification."""
    ...
    def execute(self, dataset: pd.DataFrame) -> Dict[str, Any]:
        """Execute support vector machine classification on the dataset."""
        ...
        results = {
            ...
            "accuracy": float(
                model.score(complete_data[predictor_columns].values, encoded_target)
            ),
            "support_vectors": int(model.support_.shape[0]),
            "kernel": "linear",
        }
    ...
    def explain(self, results: Dict[str, Any]) -> Dict[str, str]:
        """Return standardized explanations for SVM output keys."""
        return {
            "samples_used": "The number of complete observations used to train the support vector machine model.",
            "predictors": "The numeric predictor variables included in the support vector machine model.",
            ...
            "support_vectors": "The number of support vectors used by the SVM model.",
            "kernel": "The kernel function used by the SVM model.",
        }

    def observations(self, results: Dict[str, Any]) -> Dict[str, List[str]]:
        """Generate objective observations from SVM results."""
        if not results:
            return {"": ["Very small dataset used."]}

        observations: List[str] = []
        accuracy = results.get("accuracy", 0.0)
        support_vectors = results.get("support_vectors", 0)
        ...
        if support_vectors == 1:
            observations.append("Model uses one support vector.")
        elif support_vectors > 1:
            observations.append("Model uses multiple support vectors.")

        if results.get("samples_used", 0) < 5:
            observations.append("Very small dataset used.")

        return {"support_vector_machine": observations}
