...
from app.analysis.plugins.dbscan_analysis import DBSCANAnalysis


def test_plugin_metadata():
    plugin = DBSCANAnalysis()

    assert plugin.name == "DBSCAN Clustering"
...
def test_execute_supports_multiple_numeric_columns():
    ...
    assert results["cluster_count"] == len(results["cluster_sizes"])
    assert results["cluster_count"] >= 0
    assert results["noise_points"] >= 0
    assert sum(results["cluster_sizes"].values()) + results["noise_points"] == results["samples_used"]
...
def test_execute_returns_python_native_types_only():
    ...
    assert isinstance(results["noise_points"], int)
    assert isinstance(results["cluster_sizes"], dict)
...
def test_observations_returns_expected_structure():
    ...
    assert "dbscan" in observations
    assert isinstance(observations["dbscan"], list)
    assert len(observations["dbscan"]) >= 1
...
def test_observations_handles_empty_results():
    ...
    assert isinstance(observations, dict)
    assert observations == {"": ["No clusters detected."]}
...
def test_observations_produces_objective_observations_only():
    ...
    allowed_observations = {
        ...
        "Large proportion of observations classified as noise.",
    ...
    assert all(
        ...
        for item in observations["dbscan"]
    )
...
def test_selected_by_analysis_registry_for_numeric_dataset():
    ...
    assert "DBSCAN Clustering" in applicable_names
