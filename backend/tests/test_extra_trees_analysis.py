...
def test_plugin_metadata():
    plugin = ExtraTreesAnalysis()

    assert plugin.name == "Extra Trees"
    assert plugin.description == "Builds an Extra Trees classifier for binary classification."
...
def test_observations_returns_expected_structure():
    ...
    assert "extra_trees" in observations
    assert isinstance(observations["extra_trees"], list)
    assert len(observations["extra_trees"]) >= 1
...
def test_selected_by_analysis_registry_for_valid_dataset():
    ...
    assert "Extra Trees" in applicable_names
