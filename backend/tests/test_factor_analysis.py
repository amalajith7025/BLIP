...
def test_plugin_metadata():
    ...
    assert plugin.name == "Factor Analysis"
...
def test_execute_returns_expected_structure():
    ...
    assert set(results.keys()) == {"samples_used", "features", "components", "loadings", "noise_variance"}
...
def test_execute_supports_multiple_numeric_columns():
    ...
    assert set(results["loadings"].keys()) == set(results["features"])
    assert isinstance(results["noise_variance"], list)
...
def test_execute_returns_python_native_types_only():
    ...
    assert isinstance(results["loadings"], dict)
    assert isinstance(results["noise_variance"], list)
...
def test_explain_returns_required_keys():
    ...
    assert set(explanation.keys()) == {"samples_used", "features", "components", "loadings", "noise_variance"}
...
def test_observations_returns_expected_structure():
    ...
    assert "factor_analysis" in observations
...
def test_registered_in_analysis_engine():
    ...
    assert "Factor Analysis" in applicable_names
