...
def test_plugin_metadata():
    plugin = GradientBoostingAnalysis()

    assert plugin.name == "Gradient Boosting"
...
def test_execute_supports_multiple_numeric_predictor_columns():
    ...
    assert isinstance(results["feature_importances"], dict)
    assert pytest.approx(sum(results["feature_importances"].values()), rel=1e-6) == 1.0
    assert 0.0 <= results["accuracy"] <= 1.0
    assert isinstance(results["estimators"], int)
    assert isinstance(results["learning_rate"], float)
...
def test_execute_returns_python_native_types_only():
    ...
    assert all(
        ...
        for value in [results["samples_used"], results["estimators"]]
    )
    assert all(
        not isinstance(value, np.floating)
        for value in [results["accuracy"], results["learning_rate"]]
    )
    assert all(
        isinstance(value, float) and not isinstance(value, np.floating)
        for value in results["feature_importances"].values()
    )
...
def test_observations_returns_expected_structure():
    ...
    assert "gradient_boosting" in observations
    assert isinstance(observations["gradient_boosting"], list)
    assert len(observations["gradient_boosting"]) >= 1
...
def test_observations_produces_objective_observations_only():
    ...
    assert all(
        isinstance(item, str)
        for item in observations["gradient_boosting"]
    )
...
def test_selected_by_analysis_registry_for_valid_dataset():
    ...
    assert "Gradient Boosting" in applicable_names
