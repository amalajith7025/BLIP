...
def test_plugin_metadata():
    plugin = IndependentComponentAnalysis()

    assert plugin.name == "Independent Component Analysis"
...
def test_execute_returns_expected_structure():
    ...
    assert set(results.keys()) == {"samples_used", "features", "components", "mixing_matrix", "iterations", "converged"}
...
def test_execute_supports_multiple_numeric_columns():
    ...
    assert set(results["mixing_matrix"].keys()) == set(results["features"])
...
def test_execute_returns_python_native_types_only():
    ...
    assert isinstance(results["mixing_matrix"], dict)
    assert isinstance(results["iterations"], int)
    assert isinstance(results["converged"], bool)
...
def test_explain_returns_required_keys():
    ...
    assert set(explanation.keys()) == {"samples_used", "features", "components", "mixing_matrix", "iterations", "converged"}
...
def test_registered_in_analysis_engine():
    ...
    assert "Independent Component Analysis" in applicable_names
