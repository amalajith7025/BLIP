...
def test_plugin_metadata():
    ...
    assert plugin.name == "K-Nearest Neighbors"
...
def test_execute_returns_python_native_types_only():
    ...
    assert isinstance(results["neighbors"], int)
    assert 0.0 <= results["accuracy"] <= 1.0
    assert all(
        not isinstance(value, np.integer)
        for value in [results["samples_used"], results["neighbors"]]
    )
    assert all(
...
def test_observations_returns_expected_structure():
    ...
    assert "knn" in observations
    assert isinstance(observations["knn"], list)
    assert len(observations["knn"]) >= 1
...
def test_selected_by_analysis_registry_for_valid_dataset():
    ...
    assert "K-Nearest Neighbors" in applicable_names
