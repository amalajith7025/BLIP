...
def test_plugin_metadata():
    plugin = MeanShiftAnalysis()

    assert plugin.name == "Mean Shift Clustering"
...
def test_explain_returns_required_keys():
    ...
    assert set(explanation.keys()) == {"samples_used", "features", "clusters", "cluster_sizes", "cluster_centers"}
...
def test_observations_returns_expected_structure():
    ...
    assert "mean_shift" in observations
...
def test_registered_in_analysis_engine():
    ...
    assert "Mean Shift Clustering" in applicable_names
