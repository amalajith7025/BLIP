...
def test_plugin_metadata():
    plugin = NaiveBayesAnalysis()

    assert plugin.name == "Naive Bayes"
...
def test_execute_supports_multiple_numeric_predictor_columns():
    ...
    assert isinstance(results["class_priors"], dict)
    assert pytest.approx(sum(results["class_priors"].values()), rel=1e-6) == 1.0
...
def test_execute_returns_python_native_types_only():
    ...
    assert all(
        ...
        for value in results["class_priors"].values()
    )
...
def test_observations_returns_expected_structure():
    ...
    assert "naive_bayes" in observations
    assert isinstance(observations["naive_bayes"], list)
    assert len(observations["naive_bayes"]) >= 1
...
def test_observations_produces_objective_observations_only():
    ...
    assert all(
        isinstance(item, str)
        for item in observations["naive_bayes"]
    )
...
def test_selected_by_analysis_registry_for_valid_dataset():
    ...
    assert "Naive Bayes" in applicable_names
