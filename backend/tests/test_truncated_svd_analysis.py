...
def test_plugin_metadata():
    plugin = TruncatedSVDAnalysis()

    assert plugin.name == "Truncated SVD"
...
def test_execute_returns_expected_structure():
    ...
    assert set(results.keys()) == {"samples_used", "features", "components", "explained_variance_ratio", "singular_values"}
...
def test_execute_supports_multiple_numeric_columns():
    ...
    assert isinstance(results["singular_values"], list)
...
def test_explain_returns_required_keys():
    ...
    assert set(explanation.keys()) == {"samples_used", "features", "components", "explained_variance_ratio", "singular_values"}
...
def test_observations_returns_expected_structure():
    ...
    assert "truncated_svd" in observations
...
def test_registered_in_analysis_engine():
    ...
    assert "Truncated SVD" in applicable_names
